...
export function registerCryptoTools(
  ...
  server.registerTool(
    "get_crypto_candles",
    {
      ...
      description:
        "Get historical OHLCV candlestick data for a cryptocurrency pair (e.g. BINANCE:BTCUSDT).",
      inputSchema: {
        symbol: z
          .string()
          .describe(
            "Crypto symbol in Finnhub format: EXCHANGE:PAIR (e.g. BINANCE:BTCUSDT, COINBASE:BTC-USD)"
          ),
        resolution: z
    ...
    async ({ symbol, resolution, from, to }) => {
      try {
        ...
        const candles = await client.getCryptoCandles(
          symbol,
          resolution,
          fromTs,
          toTs
