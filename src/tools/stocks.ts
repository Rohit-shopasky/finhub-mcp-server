...
export function registerStockTools(
  ...
  server.registerTool(
    "get_stock_quote",
    {
      ...
      description:
        "Get a real-time stock quote including current price, daily high/low, open, previous close, and percent change.",
      inputSchema: {
        symbol: z
          .string()
          .toUpperCase()
  ...
  server.registerTool(
    "get_stock_candles",
    {
      ...
      description:
        "Retrieve historical OHLCV (Open, High, Low, Close, Volume) candlestick data for a stock symbol.",
      inputSchema: {
        symbol: z
          .string()
          .toUpperCase()
        ...
        from: z
          .string()
          .describe(
            "Start date in YYYY-MM-DD format (will be converted to Unix timestamp)"
          ),
        to: z
          .string()
          .describe(
            "End date in YYYY-MM-DD format (will be converted to Unix timestamp)"
          ),
      },
    },
    async ({ symbol, resolution, from, to }) => {
      try {
        ...
        const candles = await client.getStockCandles(
          symbol,
          resolution,
          fromTs,
          toTs
  ...
  server.registerTool(
    "get_company_profile",
    {
      ...
      description:
        "Get detailed company information including name, exchange, industry, market cap, IPO date, and website.",
      inputSchema: {
        symbol: z
          .string()
          .toUpperCase()
  ...
  server.registerTool(
    "get_basic_financials",
    {
      ...
      description:
        "Retrieve key financial metrics for a stock: P/E ratio, EPS, 52-week high/low, beta, dividend yield, and more.",
      inputSchema: {
        symbol: z
          .string()
          .toUpperCase()
  ...
  server.registerTool(
    "get_earnings",
    {
      title: "Get Earnings",
      description:
        "Get historical EPS earnings data including actual vs estimate and surprise percentage for a stock.",
      inputSchema: {
        symbol: z
          .string()
          .toUpperCase()
