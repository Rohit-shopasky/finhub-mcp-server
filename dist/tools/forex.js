import { z } from "zod";
export function registerForexTools(server, client) {
    // ── get_forex_rates ──────────────────────────────────────────────────────────
    server.registerTool("get_forex_rates", {
        title: "Get Forex Rates",
        description: "Get current foreign exchange rates relative to a base currency (e.g. USD, EUR, GBP).",
        inputSchema: {
            base: z
                .string()
                .toUpperCase()
        ...
        try {
            ...
            const otherRows = Object.entries(quote)
                ...
                .map(([c, r]) => `  ${(base + "/" + c).padEnd(10)}: ${r.toFixed(6)}`);
            const text = [
    ...
    // ── get_forex_candles ────────────────────────────────────────────────────────
    server.registerTool("get_forex_candles", {
        title: "Get Forex Candles",
        description: "Get historical OHLCV candlestick data for a forex pair (e.g. OANDA:EUR_USD).",
        inputSchema: {
            symbol: z
                .string()
                .describe("Forex pair symbol in Finnhub format (e.g. OANDA:EUR_USD, OANDA:GBP_USD)"),
            resolution: z
        ...
        try {
            ...
            const candles = await client.getForexCandles(symbol, resolution, fromTs, toTs);
