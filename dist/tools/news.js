import { z } from "zod";
export function registerNewsTools(server, client) {
    // ── get_company_news ─────────────────────────────────────────────────────────
    server.registerTool("get_company_news", {
        title: "Get Company News",
        description: "Retrieve recent news articles related to a specific company within a given date range.",
        inputSchema: {
            symbol: z
                .string()
                .toUpperCase()
    ...
    server.registerTool("get_market_news", {
        ...
        description: "Retrieve the latest general market news articles by category (general, forex, crypto, merger).",
        inputSchema: {
            category: z
                .enum(["general", "forex", "crypto", "merger"])
                .default("general")
                .describe("News category: general, forex, crypto, or merger (default: general)"),
            limit: z
                .number()
                .int()
                .min(1)
